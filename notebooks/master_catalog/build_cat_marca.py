...
SOURCE_REGISTRY = [
    ...
    },
    {
        ...
        "sql": """
            -- hierarchy_level filter NOT applied here: MARCA extraction needs all brand names
            -- regardless of hierarchy level. Deduplication handled by GROUP BY.
            -- (hierarchy_level filter is only needed in build_cat_upc.py for P0 bridge)
    ...
    },
    {
        ...
        "sql": """
            -- hierarchy_level filter removed: MARCA extraction only needs distinct brand names.
    ...
    },
    {
        ...
        "sql": """
            -- hierarchy_level filter removed: MARCA extraction only needs distinct brand names.
            -- PRDC_CD also removed: not available in Water Retail view (presentation pack only).
            SELECT TRIM(UPPER(CSTM_310589))  AS raw_variant,
                   CSTM_310589               AS brand_original,
                   COUNT(*)                  AS grain_count,
                   'CSTM_310589'             AS source_column,
