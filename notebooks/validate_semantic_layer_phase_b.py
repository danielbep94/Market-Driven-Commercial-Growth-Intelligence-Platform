...
for db, profile in CONNECTION_PROFILES.items():
    print(f"  {db}: warehouse={profile['sfWarehouse']}, role={profile['sfRole']}")
...
try:
    log("V9B_DAN — Water Retail derived brands (ITM_UNIF_BRAND_DAN — canonical output column)")
    df_n_wr_dan = run_sf_query("PRD_MEX", v9b_dan_sql, "V9B_DAN — Water Retail MARCA_DAN")
    log_df(df_n_wr_dan, "Water Retail ITM_UNIF_BRAND_DAN", n=300)
except Exception as e:
...
v10a_sql = """
SELECT
    LV2_UMB_BRD_DSC AS MARCA,
    CASE
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('ACTIVIA') THEN 'ACTIVIA'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('AGUAS FRESCAS', 'BFT AGUAS FRESCAS', 'BONAFONT_AGUASFRESCAS', 'BONAFONT AGUA FRESCAS') THEN 'AGUAS FRESCAS'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('BENEGASTRO', 'BENEG') THEN 'BENEGASTRO'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('BONAFONT', 'BONAFONT_NATURAL', 'AZUL BONAFONT') THEN 'BONAFONT'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('BONAFONT KIDS', 'BONAFONT_KIDS', 'BONAFONT KIDS NATURAL') THEN 'BONAFONT KIDS'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('TE BONAFONT', 'TÉ BONAFONT', 'BONAFONT TE', 'BONAFONT_TE', 'BONAFONT TÉ') THEN 'BONAFONT TE'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('DANMIX', 'DAN MIX', 'DANMMIX') THEN 'DANMIX'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('DANONE', 'DANONE YOGHURT', 'DANONE CREME', 'DANONE FREE', 'DANONE GRIEGO', 'DANONE FS', 'DAIRY') THEN 'DANONE'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('DANONINO', 'DANONINOLIQUIDO') THEN 'DANONINO'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('DANUP', 'DAN UP', 'DAN''UP') THEN 'DANUP'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('DANY', 'DANY DANETTE') THEN 'DANY'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('HERSHEYS', 'HERSHEY''S', 'DANONE HERSHEYS') THEN 'HERSHEYS'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('LEVITE', 'BONAFONT_LEVITE', 'BONAFONT LEVITE') THEN 'LEVITE'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('OCEAN SPRAY', 'OCEAN') THEN 'OCEAN SPRAY'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('OIKOS', 'OIKOS UHT') THEN 'OIKOS'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('SILK', 'SILK ORIG 946ML', 'SILKCHOCO190ML') THEN 'SILK'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('STOK COLD BREW', 'STOK') THEN 'STOK'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('YOPRO', 'YO PRO') THEN 'YOPRO'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('COCA COLA', 'COCA-COLA', 'COCACOLA', 'THE COCA COLA EXPORT', 'COCA COLA FEMSA') THEN 'COCA COLA'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('PEPSI', 'PEPSI COLA MEXICANA', 'PEPSICOLA MEXICANA') THEN 'PEPSI'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('LALA', 'GPO INDUSTRIAL LALA', 'LALA_BIO4', 'LALA_CREMA', 'LALA_GRIEGO', 'LALA_LECHE', 'LALA_YOGURT') THEN 'LALA'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('LONCHERA', 'PALAS', 'TABLAS', 'TAZAS', 'VASOS', 'TOALLAS', 'TUPPER', 'UTENSILIOS', 'CERAMICA', 'VAJILLA') THEN '_MERCHANDISE'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('0', 'MULTI', 'DAIRY') THEN '_UNKNOWN'
        WHEN TRIM(UPPER(LV2_UMB_BRD_DSC)) IN ('MULTIBRAND', 'MULTIBRAND DAIRY', 'MULTIBRAND DANONE', 'MULTIMARCA', 'DNP') THEN '_MULTIBRAND'
        ELSE TRIM(UPPER(LV2_UMB_BRD_DSC))
    END AS MARCA_STD,
    COUNT(*) AS ROW_COUNT
FROM PRD_MEX.MEX_DSP_OTC.V_D_ITEM
WHERE LV2_UMB_BRD_DSC IS NOT NULL
